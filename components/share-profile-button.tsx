...
export function ShareProfileButton({ artistId, artistName, variant = "icon" }: ShareProfileButtonProps) {
  const [copied, setCopied] = useState(false)

  const baseUrl = typeof window !== "undefined" ? window.location.origin : "https://abhinaypath.com"
  const profileUrl = `${baseUrl}/theatre-artists#${artistId}`
  const shareText = `Check out ${artistName}'s theatre artist profile on Abhinayपथ - India's First Platform for Theatre Artists & Backstage Professionals!`
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${artistName} - Theatre Artist Profile`,
          text: shareText,
          url: profileUrl,
        })
      } catch (err) {
