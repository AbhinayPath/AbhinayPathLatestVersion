...
interface ShareAuditionButtonProps {
  auditionId?: number
  auditionTitle?: string
  variant?: "icon" | "button"
  size?: "sm" | "default"
  shareType?: "audition" | "page"
}
...
export function ShareAuditionButton({
  auditionId,
  auditionTitle,
  variant = "icon",
  size = "sm",
  shareType = "audition",
}: ShareAuditionButtonProps) {
  const [copied, setCopied] = useState(false)

  const baseUrl = typeof window !== "undefined" ? window.location.origin : "https://theaterlink.com"
  
  const shareUrl = shareType === "audition" && auditionId 
    ? `${baseUrl}/auditions/${auditionId}`
    : `${baseUrl}/auditions`
  
  const shareText = shareType === "audition" && auditionTitle
    ? `Check out this audition: "${auditionTitle}" on Theaterlink - India's Premier Platform for Theatre Opportunities!`
    : "Discover theatre auditions, casting calls, and performance opportunities on Theaterlink - India's Premier Platform for Theatre!"
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: shareType === "audition" ? auditionTitle || "Audition Opportunity" : "Theatre Auditions",
          text: shareText,
          url: shareUrl,
        })
      } catch (err) {
      ...
      <DropdownMenuTrigger asChild>
        {variant === "icon" ? (
          <Button
            size="icon"
            variant="outline"
            className={`rounded-full bg-white/80 hover:bg-white border-gray-200 ${buttonSize}`}
            title={shareType === "audition" ? "Share Audition" : "Share Auditions Page"}
          >
          ...
          <Button 
            size="sm" 
            variant="outline" 
            className="rounded-full bg-white/80 hover:bg-white border-gray-200 text-xs sm:text-sm h-8 sm:h-9 px-3"
          >
            <Share2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5" />
            {shareType === "audition" ? "Share" : "Share Page"}
          </Button>
        )}
      </DropdownMenuTrigger>
